...
## Context
Radar includes `radar-core`, PostgreSQL, and Metabase. `radar-core` processes large financial datasets in Python 3.13 with Polars, NumPy, Numba, and multiprocessing for performance. The execution of `radar-core` should occur daily at 01:30 New York time, and on weekdays every 30 minutes from 09:31 through 16:31, with an additional execution at 15:54. PostgreSQL and Metabase will run continuously on the same VM. After each execution of `radar-core`, resources should be released until the next run begins.
